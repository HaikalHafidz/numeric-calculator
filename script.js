// ============================================================
// SCRIPT.JS — VERSI PERBAIKAN LENGKAP
// ============================================================
...
function buildMatrix(prefix) {
  ...
  // 2n kolom: [A | I]
  container.style.gridTemplateColumns = `repeat(${2 * n}, minmax(48px, 60px))`;
  let html = '';

  // Header: kolom A (a1..an) dan I (i1..in)
  for (let j = 0; j < n; j++) {
    html += `<div class="matrix-header-cell">a<sub>${j + 1}</sub></div>`;
  }
  html += `<div class="matrix-header-cell" style="color:var(--accent3);">|</div>`;
  for (let j = 0; j < n; j++) {
    html += `<div class="matrix-header-cell" style="color:var(--accent3);">I<sub>${j + 1}</sub></div>`;
  }

  // Sel input: A di kiri, I di kanan
  for (let i = 0; i < n; i++) {
    for (let j = 0; j < n; j++) {
      const val = (i === j) ? 1 : 0;
    ...
    for (let j = 0; j < n; j++) {
      const val = (i === j) ? 1 : 0;
      html += `<input type="number" class="matrix-cell" id="${prefix}-i-${i}-${j}" value="${val}" step="any" style="background:rgba(106,255,184,0.05);">`;
    }
  }
...
function getMatrixFullInv(prefix, rows, cols) {
  // Untuk invers: ambil dari [A | I]
  const M = [];
  for (let i = 0; i < rows; i++) {
    M.push([]);
    for (let j = 0; j < cols; j++) {
...
function getMatrixInvIdentity(prefix, rows, cols) {
  const M = [];
  for (let i = 0; i < rows; i++) {
    M.push([]);
    for (let j = 0; j < cols; j++) {
      const el = document.getElementById(`${prefix}-i-${i}-${j}`);
      if (!el) throw new Error(`Sel matriks identitas [${i + 1}][${j + 1}] tidak ditemukan`);
      const v = parseFloat(el.value);
      if (isNaN(v)) throw new Error(`Nilai identitas [${i + 1}][${j + 1}] tidak valid`);
      M[i].push(v);
    }
  }
  return M;
...
// ============================================================
// 2. INVERS MATRIKS — DENGAN 2n KOLOM ([A | I])
// ============================================================
function solveInvers() {
  ...
  let A, I;
  try {
    A = getMatrixFullInv('inv', n, n);
    I = getMatrixInvIdentity('inv', n, n);
  } catch (e) {
...
function solveNewtonRaphson() {
  ...
  rows.forEach((r, idx) => {
    ...
    html += `<div style="font-size:11px;color:var(--text-dim);">
      <span style="color:var(--accent);font-weight:700;">Iterasi ${r.i}:</span>
    </div>`;
    ...
    html += `f(x₀) = ${fmtDec(r.fx, 8)}<br>`;
    html += `f′(x₀) = ${fmtDec(r.dfx, 8)}<br>`;
    html += `<span style="color:var(--accent3);">x₁ = ${fmtDec(r.x, 8)} − (${fmtDec(r.fx, 8)} / ${fmtDec(r.dfx, 8)}) = ${fmtDec(r.xNew, 8)}</span><br>`;
