...
function closeMobileSidebar() {
  ...
  if (sidebar) sidebar.classList.remove('accordion-open');
}

// Fungsi untuk membuka accordion menu
function openMobileSidebar() {
  const sidebar = document.getElementById('sidebar');
  if (sidebar) sidebar.classList.add('accordion-open');
}
...
function initMobile() {
  ...
  // accordion toggle (pengganti sidebar slide)
  const accordionToggle = document.getElementById('accordionToggle');
  if (accordionToggle) {
    accordionToggle.addEventListener('click', () => {
      const sidebar = document.getElementById('sidebar');
      const isOpen = sidebar && sidebar.classList.contains('accordion-open');
      if (isOpen) {
  ...
  // Tombol menu mobile lama tidak dipakai lagi, tapi jangan ganggu.
  if (menuBtn) {
  ...
  // BIND MENU ITEMS - pastikan klik/touch/keyboard semuanya bekerja
