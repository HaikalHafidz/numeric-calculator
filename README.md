...
```
        ...
        └──────┬────────────┘
               │
        ┌──────▼────────────────────┐
   ┌───►│ Cari elemen terbesar pada  │
   │    │ kolom k (partial pivoting) │
   │    └──────┬─────────────────────┘
   │           │
   │    ┌──────▼────────────┐
   │    │ Tukar baris jika   │
   ...
   │           │
   │    ┌──────▼────────────┐    ya
   │    │ a[k][k] ≈ 0 ?      ├────────► Matriks singular → STOP (error)
   │    └──────┬────────────┘
   │         tidak
   │    ┌──────▼────────────┐
   │    │ Normalisasi baris  │
...
```
        ...
        └──────┬────────┘
               │
   ┌───────────▼─────────────────┐
   │ Partial pivoting kolom k      │
   └───────────┬──────────────────┘
               │
        ┌──────▼────────────┐  ya
        │ pivot ≈ 0 ?         ├────► Matriks singular → STOP
        └──────┬────────────┘
             tidak
        ┌──────▼────────────┐
        │ Normalisasi baris k │
...
```
        ...
        ┌──────▼──────────────────┐
        │ Input: f(x), a, b,        │
        │ toleransi, maks iterasi   │
        └──────┬────────────────────┘
               │
        ┌──────▼────────────┐  ya
        │ f(a)·f(b) > 0 ?     ├────► Interval tidak mengurung akar → STOP
        └──────┬────────────┘
             tidak
        ┌──────▼────────┐
        │ i = 1           │
        └──────┬────────┘
               │
   ┌───────────▼──────────────────────┐
   │ xᵣ = (a·f(b) − b·f(a)) / (f(b)−f(a))│
   └───────────┬───────────────────────┘
               │
        ┌──────▼────────────┐  ya
        │ |f(xᵣ)| < toleransi ├────► Akar ditemukan = xᵣ
        └──────┬────────────┘
             tidak
        ┌──────▼────────────┐
        │ f(a)·f(xᵣ) < 0 ?    │
...
```
   ...
   └───────────┬───────────────┘
               │
        ┌──────▼────────────┐  ya
        │ |f'(x)| ≈ 0 ?       ├────► Gagal konvergen → STOP (error)
        └──────┬────────────┘
             tidak
   ┌───────────▼──────────────┐
   │ xᵦₐᵣᵤ = x − f(x)/f'(x)      │
